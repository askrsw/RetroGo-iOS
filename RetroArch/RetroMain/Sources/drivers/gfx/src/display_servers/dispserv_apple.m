      ...
      int center, int monitor_index, int xoffset, int padjust)
{
   ...
   if (@available(macOS 14, *))
    ...
    if (@available(iOS 15, tvOS 15, *))
        apple_platform.displayLink.preferredFrameRateRange = CAFrameRateRangeMake(hz * 0.9, hz * 1.2, hz);
   else
#endif
       apple_platform.displayLink.preferredFramesPerSecond = hz;
#endif
   return true;
}
...
#else
   CGRect bounds = apple_platform.viewBounds;
...
#if (TARGET_OS_IOS && __IPHONE_OS_VERSION_MAX_ALLOWED >= 150000) || (TARGET_OS_TV && __TV_OS_VERSION_MAX_ALLOWED >= 150000)
   if (@available(iOS 15, tvOS 15, *))
      currentRate = apple_platform.displayLink.preferredFrameRateRange.preferred;
   else
#endif
      currentRate = apple_platform.displayLink.preferredFramesPerSecond;
#if !TARGET_OS_TV
   if (@available(iOS 15, *))
...
static void apple_display_server_set_screen_orientation(void *data, enum rotation rotation)
{
    switch (rotation)
    {
        case ORIENTATION_VERTICAL:
            [apple_platform setShouldLockCurrentInterfaceOrientation:YES];
            [apple_platform setLockInterfaceOrientation:UIInterfaceOrientationLandscapeRight];
            break;
        case ORIENTATION_FLIPPED:
            [apple_platform setShouldLockCurrentInterfaceOrientation:YES];
            [apple_platform setLockInterfaceOrientation:UIInterfaceOrientationPortraitUpsideDown];
            break;
        case ORIENTATION_FLIPPED_ROTATED:
            [apple_platform setShouldLockCurrentInterfaceOrientation:YES];
            [apple_platform setLockInterfaceOrientation:UIInterfaceOrientationLandscapeLeft];
            break;
        case ORIENTATION_NORMAL:
        default:
            [apple_platform setShouldLockCurrentInterfaceOrientation:NO];
            break;
    }
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 160000
    if (@available(iOS 16.0, *))
    {
        [apple_platform setNeedsUpdateOfSupportedInterfaceOrientations];
    }
#endif
}
...
{
    if (![apple_platform shouldLockCurrentInterfaceOrientation])
        return ORIENTATION_NORMAL;
    UIInterfaceOrientation orientation = [apple_platform lockInterfaceOrientation];
    switch (orientation)
    {
