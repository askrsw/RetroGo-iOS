...
void video_monitor_set_refresh_rate(float hz)
{
   ...
   snprintf(msg_log, sizeof(msg_log), "Video refresh rate changed to %s Hz.", rate);
      ...
      unsigned height, size_t pitch, bool is_idle)
{
   ...
   if (video_st->record_gpu_buffer)
   {
      ...
      {
         RARCH_WARN("[Recording] Viewport size calculation failed! Will continue using raw data. This will probably not work right...\n");
            ...
            || (vp.height != record_st->gpu_height))
      {
         ...
         RARCH_WARN("[Recording] Recording terminated due to resize.\n");
      ...
      unsigned height, size_t pitch)
{
   ...
   {
      video_info.current_subframe = 0;
      if (video_st->data && video_st->current_video->frame(
