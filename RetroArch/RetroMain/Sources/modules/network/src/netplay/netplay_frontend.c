   ...
   struct netplay_connection *connection, bool *had_input)
{
   ...
   RECV(header, sizeof(header[0]))
   {
      if (netplay->is_server)
      {
         ...
         RARCH_ERR("[Netplay] Failed to connect to client.\n");
         return false;
      }
      else
      {
         _msg = msg_hash_to_str(MSG_FAILED_TO_CONNECT_TO_HOST);
         RARCH_ERR("[Netplay] Failed to connect to host.\n");
         goto error;
      }
   }
   ...
   else
   {
      ...
      switch (netplay_magic)
      {
         ...
         case FULL_MAGIC:
            ...
            RARCH_ERR("[Netplay] Netplay host full.\n");
            goto error;
         case BANNED_MAGIC:
            _msg = msg_hash_to_str(MSG_NETPLAY_BANNED);
            RARCH_ERR("[Netplay] You are banned from this host.\n");
            goto error;
         default:
            _msg = msg_hash_to_str(MSG_NETPLAY_NOT_RETROARCH);
            RARCH_ERR("[Netplay] A netplay connection attempt failed because the peer is not running RetroArch, or is running an old version of RetroArch.\n");
            goto error;
      }
   }
   ...
   RECV(header + 1, sizeof(header) - sizeof(header[0]))
   {
      if (netplay->is_server)
      {
         ...
         RARCH_ERR("[Netplay] Failed to receive header from client.\n");
         return false;
      }
      else
      {
         _msg = msg_hash_to_str(MSG_FAILED_TO_RECEIVE_HEADER_FROM_HOST);
         RARCH_ERR("[Netplay] Failed to receive header from host.\n");
         goto error;
      }
   }
   ...
   if (netplay->is_server)
   {
            ...
            connection->netplay_protocol > HIGH_NETPLAY_PROTOCOL_VERSION)
      {
         ...
         RARCH_ERR("[Netplay] A netplay peer is running an old version of RetroArch. Cannot connect.\n");
         return false;
      }
   ...
   if (netplay->quirks & NETPLAY_QUIRK_PLATFORM_DEPENDENT)
   {
      if (ntohl(header[1]) != netplay_platform_magic())
      {
         ...
         RARCH_ERR("[Netplay] This core does not support netplay between different platforms\n");
   ...
   else if (netplay->quirks & NETPLAY_QUIRK_ENDIAN_DEPENDENT)
   {
      if (netplay_endian_mismatch(netplay_platform_magic(), ntohl(header[1])))
      {
         ...
         RARCH_ERR("[Netplay] This core does not support netplay between these platforms\n");
   ...
   if (ntohl(header[5]) != netplay_impl_magic())
   {
      ...
      RARCH_WARN("[Netplay] WARNING: A netplay peer is running a different version of RetroArch. If problems occur, use the same version.\n");
      ...
      struct netplay_connection *connection)
{
   ...
   if (netplay->is_server)
   {
      ...
      RARCH_LOG("[Netplay] Connection slot %u\n",
         slot);
   ...
   struct netplay_connection *connection, bool *had_input)
{
         ...
         || ntohl(nick_buf.cmd[1]) != sizeof(nick_buf.nick))
   {
      ...
      if (netplay->is_server)
         _msg =  "Failed to get nickname from client.";
      else
      {
         _msg = "Failed to receive nickname from host.";
         runloop_msg_queue_push(msg_hash_to_str(MSG_FAILED_TO_RECEIVE_NICKNAME_FROM_HOST),
            strlen(_msg), 1, 180, false, NULL,
            MESSAGE_QUEUE_ICON_DEFAULT, MESSAGE_QUEUE_CATEGORY_INFO);
      }
   ...
   struct netplay_connection *connection, bool *had_input)
{
   ...
   RECV(&info_buf, sizeof(info_buf.cmd))
   {
      if (!netplay->is_server)
      {
         ...
         RARCH_ERR("[Netplay] Incorrect password\n");
         runloop_msg_queue_push(_msg, strlen(_msg), 1, 180, false, NULL,
            MESSAGE_QUEUE_ICON_DEFAULT, MESSAGE_QUEUE_CATEGORY_INFO);
         return false;
      }
   ...
   if (system)
   {
            ...
            info_buf.core_name, system->library_name))
      {
         ...
         RARCH_ERR("[Netplay] A netplay peer is running a different core. Cannot connect.\n");
         if (!netplay->is_server)
            runloop_msg_queue_push(_msg, strlen(_msg), 1, 180, false, NULL,
            ...
            info_buf.core_version, my_core_version))
      {
         ...
         RARCH_WARN("[Netplay] WARNING: A netplay peer is running a different version of the core. If problems occur, use the same version.\n");
   ...
   if (content_crc && ntohl(info_buf.content_crc) != content_crc)
   {
      ...
      const char *_msg_log = netplay->modus == NETPLAY_MODUS_CORE_PACKET_INTERFACE ? "Host is running a different game." : "Content CRC32s differ. Cannot use different games.";
   ...
   struct netplay_connection *connection, bool *had_input)
{
   ...
   RECV(cmd, sizeof(cmd))
   {
      ...
      RARCH_ERR("[Netplay] Your ping is too high for this host.\n");
      runloop_msg_queue_push(_msg, strlen(_msg), 1, 180, false, NULL,
         MESSAGE_QUEUE_ICON_DEFAULT, MESSAGE_QUEUE_CATEGORY_INFO);
      return false;
   }
   ...
   if (local_sram_size && local_sram_size == remote_sram_size)
   {
      ...
      {
         free(sram_buf);
         RARCH_ERR("[Netplay] Failed to receive SRAM data from host.\n");
         return false;
      }
...
// #include "network/netplay/netplay_keys.h"
    K(BACKSPACE)
    K(TAB)
    KL(LINEFEED, 10)
    K(CLEAR)
               ...
               for (i = 0; i < MAX_INPUT_DEVICES; i++)
               {
                  ...
                  if (!netplay->device_share_modes[i] || !share_mode)
                  {
                     ...
                     else
                     {
                        ...
                        RARCH_LOG("[Netplay] The input devices requested are not available\n");
                        runloop_msg_queue_push(_msg, strlen(_msg), 1, 180, false, NULL,
                           MESSAGE_QUEUE_ICON_DEFAULT, MESSAGE_QUEUE_CATEGORY_INFO);
                     }
                     return;
               ...
               if (i >= MAX_INPUT_DEVICES)
               {
                  ...
                  else
                  {
                     ...
                     else
                     {
                        ...
                        RARCH_LOG("[Netplay] There are no free player slots\n");
                        runloop_msg_queue_push(_msg, strlen(_msg), 1, 180, false, NULL,
                           MESSAGE_QUEUE_ICON_DEFAULT, MESSAGE_QUEUE_CATEGORY_INFO);
                     }
                     return;
      ...
      default:
         {
            ...
            RARCH_WARN("[Netplay] Unknown netplay command received\n");
         }
         break;
   }
      ...
      uint16_t ext_port)
{
   ...
   if (net_st->nat_traversal_request.status == NAT_TRAVERSAL_STATUS_OPENED)
   {
      ...
      else
      {
         ...
         RARCH_WARN("[Netplay] External network has a private or shared address. Consider using a relay server.\n");
   ...
   else
   {
      ...
      RARCH_ERR("[Netplay] Netplay UPnP Port Mapping Failed\n");
      ...
      void *user_data, const char *error)
{
   ...
   if (!host_room->connectable && first)
   {
      ...
      RARCH_WARN("[Netplay] Your room is not connectable from the internet.\n");
      runloop_msg_queue_push(_msg, strlen(_msg), 1, 180, false, NULL,
            MESSAGE_QUEUE_ICON_DEFAULT, MESSAGE_QUEUE_CATEGORY_INFO);
   }
...
bool init_netplay(const char *server, unsigned port, const char *mitm_session)
{
         ...
         && modus != NETPLAY_MODUS_CORE_PACKET_INTERFACE)
   {
      ...
      RARCH_ERR("[Netplay] Core does not support netplay.\n");
...
failure:
...

   {
      ...
      RARCH_ERR("[Netplay] Failed to initialize netplay.\n");
      runloop_msg_queue_push(_msg, strlen(_msg), 0, 180, false, NULL,
            MESSAGE_QUEUE_ICON_DEFAULT, MESSAGE_QUEUE_CATEGORY_INFO);
   }
