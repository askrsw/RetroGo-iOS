      ...
      bsv_movie_t *handle, const char *path)
{
   ...
   {
      RARCH_ERR("[Replay] Input replay movie file is not a valid REPLAY file.\n");
      return false;
   }
   ...
   if (state_size)
   {
      ...
      {
         RARCH_ERR("[Replay] Could not read state from movie.\n");
         return false;
      }
      ...
      {
         RARCH_WARN("[Replay] Input replay movie format seems to have a different serializer version. Will most likely fail.\n");
      }
   }
...
static bool bsv_movie_start_record(input_driver_state_t * input_st, char *path)
{
   ...
   if (!(state = bsv_movie_init_internal(path, RARCH_MOVIE_RECORD)))
   {
      ...
      RARCH_ERR("[Replay] Failed to start movie record.\n");
      return false;
   }
...
static bool bsv_movie_start_playback(input_driver_state_t *input_st, char *path)
{
   ...
   {
      RARCH_ERR("[Replay] Failed to load movie file: \"%s\".\n",
            path);
      return false;
   }
   ...
   runloop_msg_queue_push(_msg, strlen(_msg), 2, 180, false, NULL,
         MESSAGE_QUEUE_ICON_DEFAULT, MESSAGE_QUEUE_CATEGORY_INFO);
   RARCH_LOG("[Replay] Starting movie playback.\n");
...
bool movie_stop_playback(input_driver_state_t *input_st)
{
   ...
   runloop_msg_queue_push(_msg, strlen(_msg), 2, 180, false, NULL,
         MESSAGE_QUEUE_ICON_DEFAULT, MESSAGE_QUEUE_CATEGORY_INFO);
   RARCH_LOG("[Replay] Input replay movie playback ended.\n");
...
bool movie_stop_record(input_driver_state_t *input_st)
{
   ...
   runloop_msg_queue_push(_msg, strlen(_msg), 2, 180, true, NULL,
         MESSAGE_QUEUE_ICON_DEFAULT, MESSAGE_QUEUE_CATEGORY_INFO);
   RARCH_LOG("[Replay] Stopping movie record.\n");
   bsv_movie_deinit_full(input_st);
   input_st->bsv_movie_state.flags &= ~(
         BSV_FLAG_MOVIE_END
