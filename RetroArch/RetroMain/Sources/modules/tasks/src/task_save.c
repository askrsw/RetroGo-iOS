...
bool content_undo_load_state(void)
{
   ...
   char log_undo_load_buf[PATH_MAX_LENGTH] = { 0 };

   if (!core_info_current_supports_savestate())
   {
      RARCH_LOG("[State] Core does not support save states.\n");
      return false;
   }

   RARCH_LOG("[State] Loading state \"%s\", %u bytes.\n",
         shorten_path_for_log(undo_load_buf.path, log_undo_load_buf, sizeof(log_undo_load_buf)),
   ...
   {
      RARCH_LOG("[SRAM] Blocking SRAM Overwrite.\n");
   ...
   if (!ret)
   {
      RARCH_ERR("[State] Failed to undo load state. \"%s\".\n",
         undo_load_buf.path);
      return false;
   }
      ...
      void *user_data, const char *error)
{
   ...
   struct sram_block *blocks   = NULL;
   struct string_list *savefile_list = (struct string_list*)savefile_ptr_get();
   char log_load_data[PATH_MAX_LENGTH] = { 0 };
   ...
   RARCH_LOG("[State] Loading state \"%s\", %u bytes.\n",
         shorten_path_for_log(load_data->path, log_load_data, sizeof(log_load_data)),
         (unsigned)_len);
...
error:
   RARCH_ERR("[State] Failed to load state from \"%s\".\n",
         load_data->path);
   if (buf)
      free(buf);
   free(load_data);
...
// haharsw added
bool content_direct_save_state(const char *path) {
    size_t _len;
    void *data  = NULL;
    ...
    if (!(data = content_get_serialized_data(&_len))) {
        RARCH_ERR("[State] Failed to save state to \"%s\".\n", path);
        return false;
    }

    RARCH_LOG("[State] Saving state \"%s\", %u bytes.\n", shorten_path_for_log(path, log_path, sizeof(log_path)), (unsigned)_len);
...
bool content_save_state(const char *path, bool save_to_disk)
{
   ...
   {
      const char *p_path = shorten_path_for_log(path, log_path, sizeof(log_path));
      if (!(data = content_get_serialized_data(&_len)))
      {
         RARCH_ERR("[State] Failed to save state to \"%s\".\n",
               p_path);
   ...
   if (save_to_disk)
   {
      if (path_is_valid(path))
      {
         ...
         RARCH_LOG("[State] File already exists. Saving to backup buffer...\n");
...
bool content_load_state_from_ram(void)
{
   ...
   RARCH_LOG("[State] Loading state, %u bytes.\n",
   ...
   if (!ret)
   {
      RARCH_ERR("[State] Failed to load SRAM.\n");
      return false;
   }
...
bool content_save_state_to_ram(void)
{
   ...
   if (!save_state_in_background)
   {
      if (!(data = content_get_serialized_data(&_len)))
      {
         RARCH_ERR("[State] Failed to save SRAM.\n");
         return false;
      }

      RARCH_LOG("[State] Saving state, %u bytes.\n", (unsigned)_len);
   }
