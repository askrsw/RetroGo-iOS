...
static int task_database_cue_get_serial(const char *name, char *s, size_t len)
{
   ...
   {
#ifdef DEBUG
      RARCH_LOG("[Scanner] Could not find valid data track\n");
#endif
      return 0;
   }
   ...
   switch (dbinfo->status)
   {
      case DATABASE_STATUS_ITERATE_BEGIN:
         if (dbstate && !dbstate->list)
         {
            ...
            RARCH_LOG("[Scanner] Scanning content: \"%s\"...\n", db->fullpath);
            if (retroarch_override_setting_is_set(RARCH_OVERRIDE_SETTING_DATABASE_SCAN, NULL))
               printf("Scanning content: \"%s\"...\n", db->fullpath);
      ...
      retro_task_callback_t cb)
{
   ...
   playlist_config_set_base_content_directory(&db->playlist_config, settings->bools.playlist_portable_paths ? settings->paths.directory_start : NULL);
#else
   db->playlist_config.capacity            = COLLECTION_SIZE;
