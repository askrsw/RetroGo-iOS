...
bool get_screenshot_data(uint8_t **png_data, uint64_t *png_data_size) {
    ...
    // take_screenshot_choice -> if(supports_vp_read)
    /* Avoid taking screenshot of GUI overlays. */
    if (video_st->poke && video_st->poke->set_texture_enable)
       video_st->poke->set_texture_enable(video_st->data, false, false);
    if (!(runloop_flags & RUNLOOP_FLAG_IDLE))
       video_driver_cached_frame();
    ...
    if (!(   video_st->current_video->read_viewport
          && video_st->current_video->read_viewport(video_st->data, buffer, runloop_flags & RUNLOOP_FLAG_IDLE))) {
        ret = false;
        goto end;
    }
    /* Limit image to screen size */
    ...
    // ret = screenshot_dump_direct(state);
    {
        uint8_t *png_buf = (uint8_t*)malloc(vp.width * vp.height * 3);
        struct scaler_ctx scaler = { 0 };
        scaler.in_fmt = SCALER_FMT_BGR24;
        video_frame_convert_to_bgr24(&scaler, png_buf, (const uint8_t*)buffer + ((int)vp.height - 1) * vp.width * 3, vp.width, vp.height, -vp.width * 3);
        scaler_ctx_gen_reset(&scaler);
        *png_data = rpng_save_image_bgr24_string(png_buf, vp.width, vp.height, vp.width * 3, png_data_size);
        free(png_buf);
    }
