      ...
      content_file_list_t *file_list)
{
   ...
   if (file_list->temporary_files)
   {
      ...
      for (i = 0; i < file_list->temporary_files->size; i++)
      {
         ...
         RARCH_LOG("[Content] Removing temporary content file: \"%s\".\n",
               path);

         if (filestream_delete(path) != 0)
            RARCH_ERR("[Content] Failed to remove temporary file: \"%s\".\n",
                  path);
      }
      ...
      size_t *data_size)
{
   ...
   *data_size            = 0;
   ...
   RARCH_LOG("[Content] Loading content file: \"%s\".\n",
...
#endif
            RARCH_LOG("[Content] Content loading skipped. Implementation will load it on its own.\n");
      ...
      int *argc, char **argv)
{
   *argc = 0;
   ...
   {
      char log_content_path[PATH_MAX_LENGTH] = { 0 };
      RARCH_LOG("[Core] Using content: \"%s\".\n", shorten_path_for_log(args->content_path, log_content_path, sizeof(log_content_path)));
      ...
      content_information_ctx_t *content_ctx)
{
   ...
   char log_system_dir[PATH_MAX_LENGTH];
   ...
   RARCH_LOG("[Content] Updating firmware status for: \"%s\" on \"%s\".\n",
         ...
         shorten_path_for_log(firmware_info.directory.system,
               log_system_dir, sizeof(log_system_dir)));
...
bool task_push_start_dummy_core(content_ctx_info_t *content_info)
{
   ...
   runloop_state_t *runloop_st             = runloop_state_get_ptr();
   rarch_system_info_t *sys_info           = &runloop_st->system;
   const char *path_dir_system             = core_info_get_current_firmwares_path();
      ...
      void *user_data)
{
   ...
   runloop_state_t *runloop_st                = runloop_state_get_ptr();
   bool check_firmware_before_loading         = settings->bools.check_firmware_before_loading;
   const char *path_dir_system                = NULL;
#ifndef HAVE_DYNAMIC
   ...
   path_dir_system = core_info_get_current_firmwares_path();
   if (!string_is_empty(path_dir_system))
      content_ctx.directory_system = strdup(path_dir_system);
