...
bool autosave_init(bool compress_files, unsigned autosave_interval)
{
   ...
   for (i = 0; i < task_save_files->size; i++)
   {
      ...
      if (!(auto_st = autosave_new(path,
            ...
            compress_files)))
      {
         RARCH_WARN("[SRAM] Could not initialize autosave.\n");
         continue;
      }
...
static bool content_save_ram_file(unsigned slot, bool compress)
{
   ...
   RARCH_LOG("[SRAM] Saving RAM type #%u to \"%s\".\n",
         ram.type,
         ram.path);
   ...
   RARCH_LOG("[SRAM] Saved successfully to \"%s\".\n",
         ram.path);

   return true;

fail:
   RARCH_ERR("[SRAM] Failed to save SRAM.\n");
