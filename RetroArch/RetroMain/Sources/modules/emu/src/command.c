...
command_t* command_network_new(uint16_t port)
{
   ...
   RARCH_LOG("[NetCMD] Bringing up command interface on port %hu.\n",
         (unsigned short)port);
   ...
   {
      RARCH_ERR("[NetCMD] Failed to bind socket.\n");
      goto error;
   }
...
command_t* command_uds_new(void)
{
   ...
   socklen_t addrsz = offsetof(struct sockaddr_un, sun_path) + STRLEN_CONST("Data/cmd") + 1;
   ...
   strcpy(&addr.sun_path[1], "Data/cmd");
...
bool command_network_send(const char *cmd_)
{
   ...
   if (cmd)
   {
      ...
      RARCH_LOG("[NetCMD] Sending command: \"%s\" to %s:%hu.\n",
...
size_t command_event_save_auto_state(void)
{
   ...
   if (content_auto_save_state((const char*)savestate_name_auto))
	   RARCH_LOG("[State] Auto save state to \"%s\" %s.\n",
...
bool command_event_load_entry_state(settings_t *settings)
{
   ...
   RARCH_LOG("[State] Found entry state in \"%s\".\n",
         entry_state_path);
   RARCH_LOG("[State] Loading entry state from \"%s\" %s.\n",
...
void command_event_load_auto_state(void)
{
   ...
   RARCH_LOG("[State] Found auto save state in \"%s\".\n",
         savestate_name_auto);

   if ((content_load_state(savestate_name_auto, false, true)))
      RARCH_LOG("[State] Auto-loading save state from \"%s\" %s.\n",
            savestate_name_auto, "succeeded");
   else
      RARCH_LOG("[State] Auto-loading save state from \"%s\" %s.\n",
            savestate_name_auto, "failed");
...
void command_event_set_savestate_auto_index(settings_t *settings)
{
   ...
   RARCH_LOG("[State] Found last state slot: #%d.\n",
         max_idx);
}
...
void command_event_set_replay_auto_index(settings_t *settings)
{
   ...
   if (max_idx)
      RARCH_LOG("[Replay] Found last replay slot: #%d\n",
            max_idx);
}
      ...
      const char *rarch_path_config)
{
   ...
   if (_len == 0)
   {
      ...
      runloop_msg_queue_push(_msg, strlen(_msg), 1, 180, true, NULL,
            MESSAGE_QUEUE_ICON_DEFAULT, MESSAGE_QUEUE_CATEGORY_INFO);
      RARCH_ERR("[Config] Config directory not set. Cannot save new config.\n");
      return false;
   }
   ...
   if (path_is_valid(core_path))
   {
      ...
      RARCH_LOG("[Config] Using core name for new config.\n");
   ...
   if (!new_path_available)
   {
      ...
      RARCH_WARN("[Config] Cannot infer new config path. Use current time.\n");
