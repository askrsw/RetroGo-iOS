...
static void state_manager_push_do(state_manager_t *state)
{
   ...
   if (state->thisblock_valid)
   {
      ...
      {
         RARCH_ERR("[Rewind] Buffer capacity insufficient.\n");
         return;
      }
      ...
      unsigned rewind_buffer_size)
{
   ...
   {
      RARCH_ERR("[Rewind] Rewind unavailable because this core lacks serialized save state support.\n");
      return;
   }

   if (audio_driver_has_callback())
   {
      RARCH_ERR("[Rewind] Implementation uses threaded audio. Cannot use rewind.\n");
      return;
   }
   ...
   if (!rewind_st->size)
   {
      RARCH_ERR("[Rewind] Failed to initialize rewind buffer. Rewinding will be disabled.\n");
      return;
   }

   RARCH_LOG("[Rewind] Initializing rewind buffer with size: %u MB\n",
   ...
   if (!rewind_st->state)
      RARCH_WARN("[Rewind] Failed to initialize rewind buffer. Rewinding will be disabled.\n");
