      ...
      bool use_secondary)
{
   ...
   if (!(runloop_st->flags & RUNLOOP_FLAG_RUNAHEAD_SAVE_STATE_SIZE_KNOWN))
   {
      ...
      if (!core_info_current_supports_runahead())
      {
          ...
          * execution */
         RARCH_WARN("[Run-Ahead] Run-Ahead unavailable because this core lacks deterministic save state support.\n");
         goto force_input_dirty;
      }
      ...
      if (!runahead_create(runloop_st))
      {
         ...
         RARCH_WARN("[Run-Ahead] Run-Ahead has been disabled because this core does not support save states.\n");
         goto force_input_dirty;
      }
   }
      ...
      for (frame_number = 0; frame_number <= runahead_count; frame_number++)
      {
         ...
         if (frame_number == 0)
         {
            if (!runahead_save_state(runloop_st))
            {
               ...
               runloop_msg_queue_push(_msg, strlen(_msg), 0, 3 * 60, true, NULL,
                     MESSAGE_QUEUE_ICON_DEFAULT, MESSAGE_QUEUE_CATEGORY_INFO);
               RARCH_WARN("[Run-Ahead] Failed to save state. Run-Ahead has been disabled.\n");
               return;
            }
         }
         ...
         if (last_frame)
         {
            if (!runahead_load_state(runloop_st))
            {
               ...
               runloop_msg_queue_push(_msg, strlen(_msg), 0, 3 * 60, true, NULL,
                     MESSAGE_QUEUE_ICON_DEFAULT, MESSAGE_QUEUE_CATEGORY_INFO);
               RARCH_WARN("[Run-Ahead] Failed to load state. Run-Ahead has been disabled.\n");
               return;
            }
         }
      }
   ...
   else
   {
      ...
      if (!secondary_core_ensure_exists(runloop_st, config_get_ptr()))
      {
         ...
         runloop_msg_queue_push(_msg, strlen(_msg), 0, 3 * 60, true, NULL,
               MESSAGE_QUEUE_ICON_DEFAULT, MESSAGE_QUEUE_CATEGORY_INFO);
         RARCH_WARN("[Run-Ahead] Failed to create second instance. Run-Ahead will now use only one instance.\n");
         goto force_input_dirty;
      }
