...
static void cheat_manager_pause_cheevos(void)
{
   ...
   RARCH_LOG("[Cheats] A cheat was activated. Achievements Hardcore Mode disabled for the current session.\n");
}
#endif
...
void cheat_manager_apply_cheats(bool notification_show_cheats_applied)
{
   ...
   if (cheat_st->size > 0 && notification_show_cheats_applied)
   {
      ...
      runloop_msg_queue_push(msg, _len, 1, 180, true, NULL,
            MESSAGE_QUEUE_ICON_DEFAULT, MESSAGE_QUEUE_CATEGORY_INFO);
      RARCH_LOG("[Cheats] Applying cheat changes.\n");
   }
...
void cheat_manager_load_game_specific_cheats(const char *path_cheat_database)
{
            ...
            false))
   {
      if (cheat_manager_load(cheat_file, true))
      {
         ...
         RARCH_LOG("[Cheats] Load game-specific cheatfile: \"%s\".\n", shorten_path_for_log(cheat_file, log_cheat_file, sizeof(log_cheat_file)));
      }
   }
}
