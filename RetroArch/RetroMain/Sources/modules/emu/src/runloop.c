      ...
      char *s2, size_t len2)
{
   ...
   if (   game_specific_options
       ...
       && path_is_valid(s))
   {
      RARCH_LOG("[Core] Game-specific core options found at \"%s\".\n",
            s);
   ...
   /* Check whether folder-specific options exist */
            ...
            && path_is_valid(s))
   {
      RARCH_LOG("[Core] Folder-specific core options found at \"%s\".\n",
            s);
      /* Notify system that we have a valid core options
       * override */
      path_set(RARCH_PATH_CORE_OPTIONS, s);
...
bool runloop_environment_cb(unsigned cmd, void *data)
{
   ...
   switch (cmd)
   {
      ...
      case RETRO_ENVIRONMENT_GET_SYSTEM_DIRECTORY:
         {
            const char *dir_system          = NULL;
            bool systemfiles_in_content_dir = settings->bools.systemfiles_in_content_dir;
            core_info_t *current_core;
            if(core_info_get_current_core(&current_core)) {
               dir_system = core_info_get_firmwares_path(current_core, true);
            }
            ...
            {
               char log_dir_system[PATH_MAX_LENGTH] = { 0 };
               *(const char**)data = dir_system;
               RARCH_LOG("[Environ] GET_SYSTEM_DIRECTORY: \"%s\".\n",
                         shorten_path_for_log(dir_system, log_dir_system, sizeof(log_dir_system)));
            }
         }
         break;
               ...
               if (log_level == RETRO_LOG_DEBUG)
               {
                  ...
                  for (p = 0; p < input_driver_max_users; p++)
                  {
                     ...
                     for (retro_id = 0; retro_id < RARCH_FIRST_CUSTOM_BIND; retro_id++)
                     {
                        ...
                        RARCH_DBG("      \"%s\" => \"%s\"\n",
                              msg_hash_to_str_us(MENU_ENUM_LABEL_VALUE_INPUT_JOYPAD_B + bind_index),
                              description);
                     }
                     ...
                     for (retro_id = RARCH_FIRST_CUSTOM_BIND; retro_id < RARCH_ANALOG_BIND_LIST_END; retro_id++)
                     {
                        ...
                        RARCH_DBG("      \"%s\" => \"%s\"\n",
                              msg_hash_to_str_us(MENU_ENUM_LABEL_VALUE_INPUT_ANALOG_LEFT_X_PLUS + bind_index - RARCH_FIRST_CUSTOM_BIND),
                              description);
                     }
                  }
               }
...
#endif // defined(HAVE_OPENGL) || defined(HAVE_OPENGL_CORE)
      ...
      case RETRO_ENVIRONMENT_GET_CORE_ASSETS_DIRECTORY:
      {
         ...
         const char *dir_core_assets = NULL;
         core_info_t *current_core;
         if(core_info_get_current_core(&current_core)) {
            dir_core_assets = core_info_get_assets_path(current_core, true);
         }
      ...
      {
         const char **dir            = (const char**)data;
         const char *dir_content     = settings->paths.directory_start;
      ...
      bool *load_no_content)
{
   ...
   {
      RARCH_ERR("[Core] Failed to open libretro core: \"%s\"\n",
            path);
      ...
      case CORE_TYPE_PLAIN:
         {
...
#endif
            {
               ...
               char log_path[PATH_MAX_LENGTH] = { 0 };
               ...
               RARCH_LOG("[Core] Loading dynamic libretro core from: \"%s\".\n",
                     shorten_path_for_log(path, log_path, sizeof(log_path)));
                           ...
                           )))
               {
                  ...
                  RARCH_ERR("[Core] Failed to open libretro core: \"%s\"\nError(s): %s\n", _msg, shorten_path_for_log(path, log_path, sizeof(log_path)), dylib_error());
         ...
         path_get(RARCH_PATH_SUBSYSTEM))))
   {
      ...
      for (i = 0; i < num_content; i++)
      {
         for (j = 0; j < info->roms[i].num_memory; j++)
         {
            ...
            RARCH_LOG("[Subsystem] Redirecting save file to \"%s\".\n",
               path);
...
static void runloop_path_init_savefile(runloop_state_t *runloop_st)
{
   ...
   {
      RARCH_LOG("[SRAM] SRAM will not be saved.\n");
      return;
   }
      ...
      input_driver_state_t *input_st)
{
   ...
   if (!event_load_save_files(runloop_st->flags &
      ...
      RARCH_LOG("[SRAM] Skipping SRAM load.\n");
...
static bool core_verify_api_version(runloop_state_t *runloop_st)
{
   ...
   RARCH_LOG("[Core] Version of libretro API: %u, Compiled against API: %u\n",
         api_version,
         RETRO_API_VERSION
         );
   return true;
      ...
      bool netplay_allow_timeskip)
{
   ...
   is_alive    = (video_st->current_video != NULL && video_st->data != NULL)
...
#endif // defined(HAVE_CG) || defined(HAVE_GLSL) || defined(HAVE_SLANG) || defined(HAVE_HLSL)
      ...
      const char *old_savestate_dir)
{
   ...
   {
      {
               ...
               && !string_is_empty(new_savefile_dir))
         {
            ...
            {
               RARCH_LOG("[Override] Reverting save file directory to %s\n",
                     intermediate_savefile_dir);
               ...
               && !string_is_empty(new_savestate_dir))
         {
            ...
            {
               RARCH_LOG("[Override] Reverting save state directory to %s\n",
                     intermediate_savestate_dir);
   ...
   if (sysinfo && !string_is_empty(sysinfo->library_name))
   {
      ...
      if (savefile_is_dir)
      {
         char log_save_file[PATH_MAX_LENGTH] = { 0 };
         fill_pathname_dir(runloop_st->name.savefile,
                           !string_is_empty(runloop_st->runtime_content_path_basename)
                           ? runloop_st->runtime_content_path_basename
                           : sysinfo->library_name,
                           ...
                           sizeof(runloop_st->name.savefile));
         RARCH_LOG("[Override] Redirecting save file to \"%s\".\n",
                   shorten_path_for_log(runloop_st->name.savefile, log_save_file, sizeof(log_save_file)));
      }

      if (savestate_is_dir)
      {
         char log_save_state[PATH_MAX_LENGTH] = { 0 };
         ...
         fill_pathname_dir(runloop_st->name.replay,
                           ...
                           sizeof(runloop_st->name.replay));
         RARCH_LOG("[Override] Redirecting save state to \"%s\".\n",
                   shorten_path_for_log(runloop_st->name.savestate, log_save_state, sizeof(log_save_state)));
      }

#ifdef HAVE_CHEATS
      if (path_is_directory(runloop_st->name.cheatfile))
      {
         char log_cheat_file[PATH_MAX_LENGTH] = { 0 };
         fill_pathname_dir(runloop_st->name.cheatfile,
               ...
               sizeof(runloop_st->name.cheatfile));
         RARCH_LOG("[Override] Redirecting cheat file to \"%s\".\n",
               shorten_path_for_log(runloop_st->name.cheatfile, log_cheat_file, sizeof(log_cheat_file)));
      }
#endif
   }
