...
void core_info_copy(core_info_t *src, core_info_t *dst)
{
   ...
   dst->notes_list                = src->notes_list                 ? string_list_clone(src->notes_list)                 : NULL;
...
static void core_info_transfer(core_info_t *src, core_info_t *dst)
{
   ...
   dst->assets_path               = src->assets_path;
   src->assets_path               = NULL;
      ...
      const char *core_exts, bool show_hidden_files)
{
   ...
   for (i = 0; i < path_list->dir_list->size; i++)
   {
      ...
      if (string_is_equal(filename, "MoltenVK.framework")) {
         continue;
      }
...
void core_info_free(core_info_t* info)
{
   size_t i;
   ...
   string_list_free(info->notes_list);
      ...
      bool *cache_supported)
{
   ...
   for (i = 0; i < path_list->core_list->size; i++)
   {
      ...
      if (string_is_equal(core_filename, "MoltenVK.framework")) {
        continue;
      }
      ...
      if((conf = core_info_get_config_file("core.info", info->path))) {
      ...
      fill_pathname_join(tmp_path, config->paths.directory_start, "cores", sizeof(tmp_path));
      fill_pathname_join(tmp_path, tmp_path, info->core_name, sizeof(tmp_path));
      info->user_path = strdup(tmp_path);
...
const char *core_info_get_assets_path(core_info_t *info, bool create) {
    if(info == NULL) {
        return NULL;
    }
    if(info->assets_path == NULL) {
        char tmp_path[DIR_MAX_LENGTH] = { 0 };
        fill_pathname_join(tmp_path, info->user_path, "assets", sizeof(tmp_path));
        info->assets_path = strdup(tmp_path);
    }
    if(create) {
        if (!path_is_directory(info->assets_path ))
            path_mkdir(info->assets_path);
    }
...
const char *core_info_get_firmwares_path(core_info_t *info, bool create) {
    ...
    if(info->firmwares_path == NULL) {
        char tmp_path[DIR_MAX_LENGTH] = { 0 };
        fill_pathname_join(tmp_path, info->user_path, "bios", sizeof(tmp_path));
        info->firmwares_path = strdup(tmp_path);
    }
    if(create) {
        if (!path_is_directory(info->firmwares_path ))
            path_mkdir(info->firmwares_path);
    }
    return info->firmwares_path;
}
