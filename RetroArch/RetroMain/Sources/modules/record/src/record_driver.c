...
bool recording_init(void)
{
   ...
   {
      RARCH_WARN("[Recording] Using libretro dummy core. Skipping recording.\n");
      return false;
   }

   if (!video_gpu_record && video_driver_is_hw_context())
   {
      RARCH_WARN("[Recording] Libretro core is hardware rendered. Must use post-shaded recording as well.\n");
      return false;
   }

   RARCH_LOG("[Recording] Custom timing given: FPS: %.2f, Sample rate: %.2f.\n",
      ...
      && video_st->current_video->read_viewport)
   {
      ...
      RARCH_LOG("[Recording] Detected viewport of %ux%u.\n",
            vp.width, vp.height);
   ...
   RARCH_LOG("[Recording] Recording to %s @ %ux%u (FB size: %ux%u pix_fmt: %u).\n",
         output,
   ...
   {
      RARCH_ERR("[Recording] Failed to start recording.\n");
      video_driver_gpu_record_deinit();
