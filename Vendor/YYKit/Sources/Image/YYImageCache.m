...
- (UIImage *)imageFromData:(NSData *)data {
    ...
    CGFloat scale = 0;
    if (scaleData) {
        NSNumber *number = [NSKeyedUnarchiver unarchivedObjectOfClass:[NSNumber self] fromData:scaleData error:nil];
...
- (void)setImage:(UIImage *)image imageData:(NSData *)imageData forKey:(NSString *)key withType:(YYImageCacheType)type {
    ...
    if (type & YYImageCacheTypeDisk) { // add to disk cache
        if (imageData) {
            if (image) {
                NSData *scaleData = [NSKeyedArchiver archivedDataWithRootObject:@(image.scale) requiringSecureCoding:NO error:nil];
                [YYDiskCache setExtendedData:scaleData toObject:imageData];
            }
            ...
            dispatch_async(YYImageCacheIOQueue(), ^{
                ...
                NSData *scaleData = [NSKeyedArchiver archivedDataWithRootObject:@(image.scale) requiringSecureCoding:NO error:nil];
                [YYDiskCache setExtendedData:scaleData toObject:data];
