...
- (NSUInteger)maximumUndoLevel {
    return _undoManager.levelsOfUndo;
}

- (void)setMaximumUndoLevel:(NSUInteger)maximumUndoLevel {
    _undoManager.levelsOfUndo = maximumUndoLevel;
}
...
- (void)_setLineSpacing:(CGFloat)lineSpacing {
...
- (void)_setParagraphSpacing:(CGFloat)paragraphSpacing {
    if(_paragraphSpacing == paragraphSpacing) return;
    [self willChangeValueForKey:@"paragraphSpacing"];
    _paragraphSpacing = paragraphSpacing;
    [self didChangeValueForKey:@"paragraphSpacing"];
}

- (void)_setParagraphStyle:(NSParagraphStyle *)paragraphStyle {
    if([_paragraphStyle isEqual:paragraphStyle]) return;
    [self willChangeValueForKey:@"paragraphStyle"];
    _paragraphStyle = paragraphStyle;
    [self didChangeValueForKey:@"paragraphStyle"];
}
...
- (void)_initTextView {
    ...
    _innerContainer.showLineNumber = NO;
...
- (void)setLineSpacing:(CGFloat)lineSpacing {
    if(_lineSpacing == lineSpacing) return;
    [self _setLineSpacing:lineSpacing];

    _typingAttributesHolder.lineSpacing = lineSpacing;
...
- (void)setParagraphSpacing:(CGFloat)paragraphSpacing {
    if(_paragraphSpacing == paragraphSpacing) return;
    [self _setParagraphSpacing:paragraphSpacing];

    _typingAttributesHolder.paragraphSpacing = paragraphSpacing;
    _innerText.paragraphSpacing = paragraphSpacing;
    [self _resetUndoAndRedoStack];
    [self _commitUpdate];
}
...
- (void)setParagraphStyle:(NSParagraphStyle *)paragraphStyle {
    if([_paragraphStyle isEqual:paragraphStyle]) return;
    [self _setParagraphStyle:paragraphStyle];

    _typingAttributesHolder.paragraphStyle = paragraphStyle;
    _innerText.paragraphStyle = paragraphStyle;
    [self _resetUndoAndRedoStack];
    [self _commitUpdate];
}
...
- (void)setAttributedText:(NSAttributedString *)attributedText {
    ...
    NSAttributedString *oldAttiburedText = _attributedText;
    ...
    // haharsw: undo-redo
    [[self.undoManager prepareWithInvocationTarget:self] setAttributedText:oldAttiburedText];
...
#pragma mark - Show line number

- (BOOL)isShowLineNumber {
    return _innerContainer.isShowLineNumber;
}

- (void)setShowLineNumber:(BOOL)showLineNumber {
    if(showLineNumber == _innerContainer.isShowLineNumber) {
        return;
    }

    [self willChangeValueForKey:@"showLineNumber"];
    _innerContainer.showLineNumber = showLineNumber;
    [self didChangeValueForKey:@"showLineNumber"];

    [self _commitUpdate];
}

- (UIColor *)lineNumberTextColor {
    return _innerContainer.lineNumberTextColor;
}

- (void)setLineNumberTextColor:(UIColor *)lineNumberTextColor {
    if([lineNumberTextColor isEqual:_innerContainer.lineNumberTextColor]) {
        return;
    }

    [self willChangeValueForKey:@"lineNumberTextColor"];
    _innerContainer.lineNumberTextColor = lineNumberTextColor;
    [self didChangeValueForKey:@"lineNumberTextColor"];
...
- (UIColor *)lineNumberBackgroundColor {
    return _innerContainer.lineNumberBackgroundColor;
}

- (void)setLineNumberBackgroundColor:(UIColor *)lineNumberBackgroundColor {
    if([lineNumberBackgroundColor isEqual:_innerContainer.lineNumberBackgroundColor]) {
        return;
    }

    [self willChangeValueForKey:@"lineNumberBackgroundColor"];
    _innerContainer.lineNumberBackgroundColor = lineNumberBackgroundColor;
    [self didChangeValueForKey:@"lineNumberBackgroundColor"];
...
- (UIFont *)lineNumberFont {
    return _innerContainer.lineNumberFont;
}

- (void)setLineNumberFont:(UIFont *)lineNumberFont {
    if([lineNumberFont isEqual:_innerContainer.lineNumberFont]) {
        return;
    }

    [self willChangeValueForKey:@"lineNumberFont"];
    _innerContainer.lineNumberFont = lineNumberFont;
    [self didChangeValueForKey:@"lineNumberFont"];
...
- (UIColor *)skipLineColor {
    return _innerContainer.skipLineColor;
}

- (void)setSkipLineColor:(UIColor *)skipLineColor {
    if([skipLineColor isEqual:_innerContainer.skipLineColor]) {
        return;
    }

    [self willChangeValueForKey:@"skipLineColor"];
    _innerContainer.skipLineColor = skipLineColor;
    [self didChangeValueForKey:@"skipLineColor"];
...
- (CGFloat)skipLineWidth {
    return _innerContainer.skipLineWidth;
}

- (void)setSkipLineWidth:(CGFloat)skipLineWidth {
    if(skipLineWidth == _innerContainer.skipLineWidth) {
        return;
    }

    [self willChangeValueForKey:@"skipLineWidth"];
    _innerContainer.skipLineWidth = skipLineWidth;
    [self didChangeValueForKey:@"skipLineWidth"];
...
- (void)layoutSubviews {
    ...
    CGSize newSize = self.bounds.size;
    BOOL changed = _innerContainer.isVerticalForm ? (_oldLayoutSize.height != newSize.height) : (_oldLayoutSize.width != newSize.width);
    if (changed) {
        [self _updateInnerContainerSize];
        [self _commitUpdate];
    }
    if (!CGSizeEqualToSize(_oldLayoutSize, newSize)) {
        [self _commitPlaceholderUpdate];
    }
    _oldLayoutSize = newSize;
}
...
- (void)setMarkedText:(NSString *)markedText selectedRange:(NSRange)selectedRange {
    ...
    NSRange oldRange = _markedTextRange ? _markedTextRange.asRange : NSMakeRange(_selectedTextRange.end.offset, 0);
    if ([self.delegate respondsToSelector:@selector(textView:shouldChangeTextInRange:replacementText:)]) {
        NSRange range = oldRange;
        BOOL should = [self.delegate textView:self shouldChangeTextInRange:range replacementText:markedText];
        if (!should) return;
    }

    NSString *oldMarkedText = [_innerText.string substringWithRange:oldRange];
    ...
    // haharsw: undo-redo
    NSRange newRange = _markedTextRange ? _markedTextRange.asRange : NSMakeRange(_selectedTextRange.end.offset, 0);
    [[self.undoManager prepareWithInvocationTarget:self] setMarkedText:oldMarkedText selectedRange:newRange];
...
- (void)replaceRange:(YYTextRange *)range withText:(NSString *)text {
    ...
    NSString *oldText = [_innerText.string substringWithRange:range.asRange];
    ...
    // haharsw: undo-redo
    NSRange newR = NSMakeRange(range.asRange.location, text.length);
    YYTextRange *newRange = [YYTextRange rangeWithRange:newR];
    [[self.undoManager prepareWithInvocationTarget:self] replaceRange:newRange withText:oldText];
