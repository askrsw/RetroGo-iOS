...
+ (instancetype)sharedWindow {
    ...
    dispatch_once(&onceToken, ^{
        UIWindowScene *foregroundScene = [UIWindow foregroundScene];
        if(foregroundScene != nil) {
            one = [[YYTextEffectWindow alloc] initWithWindowScene:foregroundScene];
        } else {
            one = [self new];
        }
...
- (UIViewController *)rootViewController {
    for (UIWindow *window in [UIWindow currentWindows]) {
...
- (void)_updateWindowLevel {
    UIWindow *top = [UIWindow currentTopWindow];
    UIWindow *key = [UIWindow currentKeyWindow];
...
- (CGFloat)_updateMagnifier:(YYTextMagnifier *)mag {
    ...
    NSMutableArray *windows = [UIWindow currentWindows].mutableCopy;
