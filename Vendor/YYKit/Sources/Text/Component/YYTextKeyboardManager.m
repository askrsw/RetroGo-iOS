...
- (UIWindow *)keyboardWindow {
    UIWindow *window = nil;
    for (window in [UIWindow currentWindows]) {
        if ([self _getKeyboardViewFromWindow:window]) return window;
    }
    window = [UIWindow currentKeyWindow];
    if ([self _getKeyboardViewFromWindow:window]) return window;

    // CGFloat version = [self _systemVersion];
    NSMutableArray *kbWindows = nil;
    for (window in [UIWindow currentWindows]) {
        NSString *windowName = NSStringFromClass(window.class);
        if([windowName isEqualToString:@"UITextEffectsWindow"] || [windowName isEqualToString:@"UIRemoteKeyboardWindow"]) {
...
- (CGRect)convertRect:(CGRect)rect toView:(UIView *)view {
    ...
    UIWindow *mainWindow = [UIWindow currentKeyWindow];
