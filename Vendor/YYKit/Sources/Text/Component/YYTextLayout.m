...
#define kDefaultLineNumberPadding 6.0f
...
@implementation YYTextContainer {
    ...
    UIColor *_lineNumberTextColor;
    UIColor *_lineNumberBackgroundColor;
    UIFont *_lineNumberFont;
    UIColor *_skipLineColor;
    CGFloat _skipLineWidth;
}
...
- (instancetype)init {
    ...
    _lineNumberFont = [UIFont fontWithName:@"Courier" size:14.0];
    return self;
}
...
- (instancetype)copyWithZone:(NSZone *)zone {
    ...
    one->_showLineNumber = _showLineNumber;
    one->_lineNumberTextColor = _lineNumberTextColor.copy;
    one->_lineNumberBackgroundColor = _lineNumberBackgroundColor.copy;
    one->_lineNumberFont = _lineNumberFont.copy;
    one->_skipLineColor = _skipLineColor.copy;
    one->_skipLineWidth = _skipLineWidth;
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    ...
    [aCoder encodeBool:_showLineNumber forKey:@"showLineNumber"];
    [aCoder encodeObject:_lineNumberTextColor forKey:@"lineNumberTextColor"];
    [aCoder encodeObject:_lineNumberBackgroundColor forKey:@"lineNumberBackgroundColor"];
    [aCoder encodeObject:_lineNumberFont forKey:@"lineNumberFont"];
    [aCoder encodeObject:_skipLineColor forKey:@"skipLineColor"];
    [aCoder encodeDouble:_skipLineWidth forKey:@"skipLineWidth"];
}

- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    self = [self init];
    _lock = dispatch_semaphore_create(1);
    _lineNumberFont = [UIFont fontWithName:@"Helvetica Neue" size:14.0];
    return self;
}
...
- (void)setVerticalForm:(BOOL)verticalForm {
    ...
    if(verticalForm && _showLineNumber) {
        self.showLineNumber = NO;
    }
}
...
- (BOOL)isShowLineNumber {
    Getter(BOOL show = _showLineNumber) return show;
}

- (void)setShowLineNumber:(BOOL)showLineNumber {
    Setter(
        if(_verticalForm && showLineNumber) {
            return;
        }
        _showLineNumber = showLineNumber;
        if(_showLineNumber) {
            if(_lineNumberTextColor == nil) {
                _lineNumberTextColor = UIColor.darkTextColor;
            }
            if(_lineNumberBackgroundColor == nil) {
                _lineNumberBackgroundColor = UIColor.clearColor;
            }
            if(_lineNumberFont == nil) {
                _lineNumberFont = [UIFont systemFontOfSize:14.0];
            }
            if(_skipLineColor == nil) {
                _skipLineColor = UIColor.grayColor;
            }
        }
    );
}
...
- (UIColor *)lineNumberTextColor {
    Getter(UIColor *c = _lineNumberTextColor) return c;
}

- (void)setLineNumberTextColor:(UIColor *)lineNumberColor {
    Setter(_lineNumberTextColor = lineNumberColor);
}

- (UIColor *)lineNumberBackgroundColor {
    Getter(UIColor *c = _lineNumberBackgroundColor) return c;
}

- (void)setLineNumberBackgroundColor:(UIColor *)lineNumberBackgroundColor {
    Setter(_lineNumberBackgroundColor = lineNumberBackgroundColor);
}

- (UIFont *)lineNumberFont {
    Getter(UIFont *f = _lineNumberFont) return f;
}

- (void)setLineNumberFont:(UIFont *)lineNumberFont {
    Setter(
        _lineNumberFont = lineNumberFont;
        if(_showLineNumber && lineNumberFont == nil) {
            _lineNumberFont = [UIFont fontWithName:@"Helvetica Neue" size:14.0];
        }
    );
}

- (UIColor *)skipLineColor {
    Getter(UIColor *c = _skipLineColor) return  c;
}

- (void)setSkipLineColor:(UIColor *)skipLineColor {
    Setter(_skipLineColor = skipLineColor);
}

- (CGFloat)skipLineWidth {
    Getter(CGFloat v = _skipLineWidth) return v;
}

- (void)setSkipLineWidth:(CGFloat)skipLineWidth {
    Setter(_skipLineWidth = skipLineWidth);
}
...
@property (nonatomic, readwrite) CGFloat lineNumberWidth;
...
+ (YYTextLayout *)layoutWithContainer:(YYTextContainer *)container text:(NSAttributedString *)text range:(NSRange)range {
    ...
    static BOOL needFixLayoutSizeBug = YES;
    ...
    UIEdgeInsets containerInsets = container.insets;
    if(container.isShowLineNumber) {
        static NSRegularExpression *regex = nil;
        if(regex == nil) {
            regex = [NSRegularExpression regularExpressionWithPattern:@"\n" options:0 error:nil];
        }
        NSString *string = text.string;
        CGFloat count = [regex numberOfMatchesInString:string options:0 range:NSMakeRange(0, string.length)] + 1;
        NSMutableString *numbers = [NSMutableString string];
        while (count >= 1) {
            [numbers appendString:@"4"];
            count /= 10;
        }

        NSDictionary *attrs = @{ NSFontAttributeName: container.lineNumberFont };
        NSAttributedString *attibuetedText = [[NSAttributedString alloc] initWithString:numbers attributes:attrs];
        CGRect rect = [attibuetedText boundingRectWithSize:CGSizeMake(0, 0) options:NSStringDrawingUsesLineFragmentOrigin context:nil];
        lineNumberWidth = kDefaultLineNumberPadding + rect.size.width + kDefaultLineNumberPadding + container.skipLineWidth;
        containerInsets.left += lineNumberWidth;
    }
    ...
    if (container.path == nil && container.exclusionPaths.count == 0) {
        ...
        if (needFixLayoutSizeBug) {
            ...
            constraintRectBeforeExtended = UIEdgeInsetsInsetRect(rect, containerInsets);
        ...
        rect = UIEdgeInsetsInsetRect(rect, containerInsets);
    ...
    { // calculate bounding size
        ...
        if (container.path) {
            ...
            rect = UIEdgeInsetsInsetRect(rect, UIEdgeInsetsInvert(containerInsets));
        }
        rect = CGRectStandardize(rect);
    ...
    CFRange cfR = CTFrameGetVisibleStringRange(ctFrame);
    visibleRange = NSMakeRange(cfR.location, cfR.length);
    if (needTruncation) {
        YYTextLine *lastLine = lines.lastObject;
    ...
    layout.lineNumberWidth = lineNumberWidth;
...
- (NSArray<YYTextLine *> *)getNewLines {
    NSString *string = self.text.string;
    NSMutableArray<YYTextLine *> *array = [NSMutableArray array];
    BOOL new = YES;
    for(YYTextLine *line in self.lines) {
        NSString *subs = [string substringWithRange:line.range];
        if(new) {
            [array addObject:line];
            new = NO;
        }
        if([subs containsString:@"\n"]) {
            new = YES;
        }
    }

    if(new && self.lines.lastObject != nil) {
        YYTextLine *line  = self.lines.lastObject;
        [array addObject:line];
    }
    ...
    NSLog(@"Match count: %ld", matches.count);

    NSInteger index = 1;
    for(YYTextLine *line in array) {
        NSString *subs = [string substringWithRange:line.range];
        if([subs containsString:@"\n"]) {
            printf("%02ld: %02ld - %02ld: %s", index, line.row, line.index, subs.UTF8String);
        } else {
            printf("%02ld: %02ld - %02ld: %s\n", index, line.row, line.index, subs.UTF8String);
        }
        index += 1;
    }
    printf("\ntotal count: %ld, new count: %ld \n", self.lines.count, array.count);
    */
...
- (BOOL)isShowLineNumber {
    return self.container.isShowLineNumber;
}

#pragma mark - Coding

- (void)encodeWithCoder:(NSCoder *)aCoder {
    NSData *textData = [YYTextArchiver archivedDataWithRootObject:_text requiringSecureCoding:NO error:nil];
    ...
    CGContextSaveGState(context); {
        if (isVertical) {
            ...
            if (styleBase == YYTextLineStyleSingle) {
                ...
                CGContextAddLineToPoint(context, x + w, y2);
                CGContextStrokePath(context);
            } else if (styleBase == YYTextLineStyleWave) {
            ...
            if (styleBase == YYTextLineStyleSingle) {
                ...
                CGFloat d1 = 0.4;                   // Distance delta
                CGFloat d2 = (20.0 / 360) * M_PI * 2.0;    // Angle delta
                CGFloat amp = w;
                CGFloat y0 = y + w * 4;
                CGContextMoveToPoint(context, x1, y0);
                for(CGFloat xx = x1 + d1, angle = 0; xx < x2; xx += d1, angle += d2) {
                    CGFloat yy = y0 + sin(angle) * amp;
...
static void YYTextDrawBlockBorder(YYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    ...
    UIEdgeInsets containerInsets = layout.container.insets;
    if(layout.isShowLineNumber) {
        containerInsets.left += layout.lineNumberWidth;
    }
    ...
    for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            if (isVertical) {
                UIEdgeInsets insets = containerInsets;
        ...
        for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            for (NSInteger ll = l; ll < lMax; ll++) {
                ...
                for (NSInteger rr = (ll == l) ? r : 0, rrMax = CFArrayGetCount(iRuns); rr < rrMax; rr++) {
                    ...
                    if (isVertical) {
                        CGFloat tmp = iRunPosition.x;
                        iRunPosition.x = iRunPosition.y;
                        iRunPosition.y = tmp;
...
static void YYTextDrawLineNumer(YYTextLayout *layout, CGContextRef context) {
    YYTextContainer *container = layout.container;
    CGContextSaveGState(context);
    if(container.lineNumberBackgroundColor != nil && [container.lineNumberBackgroundColor isEqual:UIColor.clearColor] == NO) {
        CGRect rect = CGRectMake(0, 0, layout.lineNumberWidth, layout.textBoundingSize.height);
        CGContextSetFillColorWithColor(context, container.lineNumberBackgroundColor.CGColor);
        CGContextFillRect(context, rect);
    }

    if(container.skipLineWidth > 0 && container.skipLineColor != nil) {
        CGContextSetStrokeColorWithColor(context, container.skipLineColor.CGColor);
        CGContextSetLineWidth(context, container.skipLineWidth);
        CGPoint start = CGPointMake(layout.lineNumberWidth - container.skipLineWidth * 0.5, 0);
        CGPoint end = CGPointMake(start.x, layout.textBoundingSize.height);
    ...
    UIColor *textColor = container.lineNumberTextColor ? container.lineNumberTextColor : layout.text.color;
    NSDictionary *attrs = @{
        NSFontAttributeName: container.lineNumberFont,
        NSForegroundColorAttributeName: textColor,
        NSParagraphStyleAttributeName: style
    };

    CGFloat w = layout.lineNumberWidth - kDefaultLineNumberPadding - kDefaultLineNumberPadding - container.skipLineWidth;
    NSArray<YYTextLine *> *newLines = [layout getNewLines];
    for(NSInteger index = 1; index <= newLines.count; index ++) {
        YYTextLine *line = newLines[index - 1];
        NSString *strIndex = [[NSString alloc] initWithFormat:@"%ld", index];
        NSAttributedString *attributedIndexString = [[NSAttributedString alloc] initWithString:strIndex attributes:attrs];
        CGSize strSize = [attributedIndexString boundingRectWithSize:CGSizeZero options:NSStringDrawingUsesLineFragmentOrigin context:nil].size;
        CGFloat x = kDefaultLineNumberPadding;
        CGFloat h = strSize.height;
        CGFloat y = line.top + (line.height - strSize.height) * 0.5 + line.leading;
        CGRect rect = CGRectMake(x, y, w, h);
        [attributedIndexString drawInRect:rect];
    }
    CGContextRestoreGState(context);
}
...
- (void)drawInContext:(CGContextRef)context
    ...
    @autoreleasepool {
        ...
        if (self.isShowLineNumber && context) {
            if (cancel && cancel()) return;
            YYTextDrawLineNumer(self, context);
        }
