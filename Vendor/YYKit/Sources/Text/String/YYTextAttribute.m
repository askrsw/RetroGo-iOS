...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    NSError *error = nil;
    NSData *data = [YYTextArchiver archivedDataWithRootObject:self.attributes requiringSecureCoding:NO error:&error];
    if(error != nil) {
        NSLog(@"%@", error);
        return;
    }
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    _attributes = [YYTextUnarchiver unarchivedObjectOfClass:[NSDictionary self] fromData:data error:&error];
    if(error != nil) {
        NSLog(@"%@", error);
    }
    return self;
