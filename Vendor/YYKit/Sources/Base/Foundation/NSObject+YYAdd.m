...
- (id)performSelectorWithArgsOnMainThread:(SEL)sel waitUntilDone:(NSInteger)wait, ...{
    // NSInteger mark = w
    INIT_INV(wait, nil);
...
- (id)performSelectorWithArgs:(SEL)sel onThread:(NSThread *)thr waitUntilDone:(NSInteger)wait, ...{
    INIT_INV(wait, nil);
    if (!wait) [inv retainArguments];
