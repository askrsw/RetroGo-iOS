...
RetroGo 基于 GNU General Public License v3.0（GPLv3）开源发布。
在遵循 GPLv3 许可的前提下，我们对分发、品牌及 App Store 上架行为作如下补充说明：

1. **商标与品牌**
   “RetroGo”名称、应用图标及相关品牌元素不属于 GPLv3 授权范围。
   未经明确授权，禁止在再发布版本中使用“RetroGo”名称、图标或任何可能引起混淆的相似品牌标识。

2. **App Store 再发布行为**
   尽管 GPLv3 允许再分发，我们强烈不建议以下行为：
   - 直接发布未修改的构建版本
   - 仅进行简单换壳（如更换名称、图标）的应用
   - 未提供实质性改进或新增价值的再发布版本

3. **用户混淆与冒充**
   任何可能误导用户，使其认为该应用为 RetroGo 官方版本的分发行为，
   将被视为对我们品牌权益的侵犯。

4. **维权与申诉**
   对于存在以下情形的应用，我们保留向平台方（如 Apple App Store）发起投诉与申诉的权利：
   - 侵犯品牌或商标权益
   - 造成用户混淆或误导
   - 被认定为垃圾应用、重复应用或低质量简单重打包
...
使用本项目代码即表示您同意同时遵守 GPLv3 许可及上述品牌使用规范。
